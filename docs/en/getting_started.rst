...
The WebRTC SDK submodule needs a few platform patches on top of its recorded SHA.
They are applied **automatically** at configure time (``patches/apply_patches.sh``) to
the ``amazon-kinesis-video-streams-webrtc-sdk-c`` submodule — a fresh ``--recursive``
clone plus ``idf.py build`` is enough, with no manual ``git am`` and nothing applied to
your ESP-IDF tree. To manage them yourself, see the build guide (``docs/building.md``)
...
2. Place your certificates in the SPIFFS image directory:
   ...
   .. code-block:: none
      ...
      examples/common_components/app_common/spiffs_image/certs/
