...
| **libsrtp** (registry `espressif/libsrtp`) | Secure RTP implementation | - | Media encryption |
...
#### libsrtp (registry `espressif/libsrtp`, ~2.8)
...
| **Link Errors** | libwebsockets | Ensure local components are in `EXTRA_COMPONENT_DIRS`; registry deps (e.g. `espressif/libsrtp`) resolve via `idf_component.yml` |
...
- 📖 **[docs/custom_signaling.md](../docs/custom_signaling.md)** - Custom component development
