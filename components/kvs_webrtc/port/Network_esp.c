...
static UINT32 getEspIdfLocalIpAddresses(PKvsIpAddress destIpList, UINT32 destIpListLen, IceSetInterfaceFilterFunc filter, UINT64 customData)
{
    ...
    for (UINT32 i = 0; i < netif_count && ipCount < destIpListLen; i++) {
        ...
        // Skip interfaces that aren't up — esp_netif can retain a stale IP
        // briefly after a STA disconnect, which leaks into ICE host candidates
        // and wastes the peer's connectivity-check budget on unreachable addrs.
        if (!esp_netif_is_netif_up(esp_netif)) {
            DLOGD("Interface %s not up — skipping", netif_keys[i]);
            continue;
        }
        ...
        if (ip_info.ip.addr != 0) {
            ...
            DLOGD("Acquired IPv4 for %s: %d.%d.%d.%d",
                  netif_keys[i],
        ...
        // IPv6 host candidates: enumerate all valid v6 addresses on this netif.
        // Includes link-local — KVS upstream's getifaddrs() path emits them too,
        // and they're useful for ICE on local networks. CONFIG_LWIP_IPV6 gates
        // the call so non-IPv6 builds don't trip the symbol.
        esp_ip6_addr_t ip6_list[CONFIG_LWIP_IPV6_NUM_ADDRESSES];
        int n6 = esp_netif_get_all_ip6(esp_netif, ip6_list);
        for (int j = 0; j < n6 && ipCount < destIpListLen; j++) {
            // Skip the all-zeros placeholder lwIP may return for unfilled slots.
            if (ip6_list[j].addr[0] == 0 && ip6_list[j].addr[1] == 0 &&
                ip6_list[j].addr[2] == 0 && ip6_list[j].addr[3] == 0) {
                continue;
            }
            destIpList[ipCount].isPointToPoint = 0;
            destIpList[ipCount].family = KVS_IP_FAMILY_TYPE_IPV6;
            destIpList[ipCount].port = 0;
            MEMCPY(destIpList[ipCount].address, (BYTE *) &ip6_list[j].addr[0], IPV6_ADDRESS_LENGTH);
            DLOGD("Acquired IPv6 for %s: %08x:%08x:%08x:%08x", netif_keys[i],
                  ip6_list[j].addr[0], ip6_list[j].addr[1],
                  ip6_list[j].addr[2], ip6_list[j].addr[3]);
            ipCount++;
        }
#endif
    }
...
static STATUS setEspSocketBufferSize(INT32 sockfd, UINT32 sendBufSize)
{
    ...
    // All ESP-IDF targets share the same lwIP stack — SO_SNDBUF is honored
    // up to CONFIG_LWIP_TCP_SND_BUF_DEFAULT. Try it on every target and
    // treat failure as non-fatal (lwIP may silently cap or reject larger
    // sizes; the connection still works at the default size).
    if (setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, &sendBufSize, SIZEOF(sendBufSize)) < 0) {
        DLOGW("setsockopt() SO_SNDBUF=%u failed (errno %s) — using lwIP default",
              sendBufSize, getErrorString(getErrorCode()));
    }
...
STATUS getLocalhostIpAddresses(PKvsIpAddress destIpList, PUINT32 pDestIpListLen, IceSetInterfaceFilterFunc filter, UINT64 customData)
{
    ...
    /* Used only by the _WIN32 / non-IDF arms below; unused on ESP-IDF. */
    BOOL filterSet __attribute__((unused)) = TRUE;
...
STATUS socketBind(PKvsIpAddress pHostIpAddress, INT32 sockfd)
{
    ...
    pHostIpAddress->port = (pHostIpAddress->family == KVS_IP_FAMILY_TYPE_IPV4) ? (UINT16) ipv4Addr.sin_port : (UINT16) ipv6Addr.sin6_port;
...
{
    /* sscanf %u / %x bind to `unsigned int *` exactly; on xtensa the KVS
     * UINT32 typedef is `long unsigned int`, so feeding UINT32* to %u
     * trips -Wformat=. Use plain unsigned int locals — values cap at
     * 0xFFFF so 16-bit range is irrelevant. %n is an int*. */
    int offset = 0;
    unsigned int ip_1, ip_2, ip_3, ip_4, ip_5, ip_6, ip_7, ip_8;
