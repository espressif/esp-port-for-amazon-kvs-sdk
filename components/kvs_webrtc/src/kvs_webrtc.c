...
static VOID kvs_videoPictureLossHandler(UINT64 customData);
...
static WEBRTC_STATUS kvs_pc_set_ice_servers(void *pPeerConnectionClient, void *ice_servers, uint32_t ice_count)
{
    ...
    if (ice_servers != NULL && ice_count > 0) {
        // ice_servers is app_webrtc_ice_server_t[] (urls[128]); the KVS
        // RtcIceServer has urls[256] — different layout. Casting and bulk-
        // copying as RtcIceServer misaligns every entry after [0] and blanks
        // TURN URLs (relay fails on cellular). Copy field-by-field, bounded by
        // the destination (RtcIceServer) field sizes.
        app_webrtc_ice_server_t *src = (app_webrtc_ice_server_t *) ice_servers;
        RtcIceServer *dst = (RtcIceServer *) MEMCALLOC(ice_count, SIZEOF(RtcIceServer));
        if (dst != NULL) {
            for (UINT32 i = 0; i < ice_count; i++) {
                STRNCPY(dst[i].urls, src[i].urls, SIZEOF(dst[i].urls) - 1);
                dst[i].urls[SIZEOF(dst[i].urls) - 1] = '\0';
                STRNCPY(dst[i].username, src[i].username, SIZEOF(dst[i].username) - 1);
                dst[i].username[SIZEOF(dst[i].username) - 1] = '\0';
                STRNCPY(dst[i].credential, src[i].credential, SIZEOF(dst[i].credential) - 1);
                dst[i].credential[SIZEOF(dst[i].credential) - 1] = '\0';
...
static STATUS kvs_setupMediaTracks(kvs_pc_session_t* session)
{
    ...
    // Respond to remote Picture Loss Indications (PLI) by forcing a keyframe so a
    // packet-loss-stalled decoder resyncs immediately instead of waiting for the periodic GOP.
    CHK_STATUS(transceiverOnPictureLoss(session->video_transceiver, POINTER_TO_HANDLE(session), kvs_videoPictureLossHandler));
...
static VOID kvs_videoPictureLossHandler(UINT64 customData)
{
    ...
    /* Fires in the RTCP receive task. Just request a keyframe; the encoder task
     * forces the IDR on its next frame (see video_capture_request_keyframe). */
    esp_err_t err = video_capture_request_keyframe();
    ESP_LOGW(TAG, "PLI from peer %s -> requesting keyframe (%s)",
             session ? session->peer_id : "?", esp_err_to_name(err));
}
