...
/**
 * @brief Purge all queued messages but keep the mutex alive for reuse.
 *
 * Safe to call across connect/disconnect cycles; the mutex is created once by
 * message_queue_init() and persists until message_queue_deinit().
 *
 * @param queue Pointer to the message queue to purge
 * @return esp_err_t ESP_OK on success, error code otherwise
 */
esp_err_t message_queue_cleanup(message_queue_t *queue);

/**
 * @brief Fully tear down a message queue (purge + delete the mutex).
 *
 * Only for final teardown of the owning client; after this the queue must be
 * message_queue_init()'d again before use.
 *
