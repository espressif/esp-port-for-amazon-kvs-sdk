...
static esp_err_t get_websocket_url(const char *room_id, char **wss_url)
{
    ...
    // The server reports the join outcome in "result". On "FULL" (the room
    // already has two peers) or any non-SUCCESS value, params omits client_id /
    // wss_url, so we must stop here — proceeding would register/offer on NULL
    // fields and crash. Bail gracefully; the caller treats this as a failed join.
    cJSON *result_item = cJSON_GetObjectItem(root, "result");
    if (result_item && cJSON_IsString(result_item) &&
        strcmp(result_item->valuestring, "SUCCESS") != 0) {
        ESP_LOGE(TAG, "Room join rejected by server: result=%s (room full or unavailable)",
    ...
    // Only report success if the fields we actually need arrived. Without a
    // client_id (registration) and wss_url (the collider socket) there is
    // nothing to connect to — return an error rather than proceeding on NULLs.
    if (apprtc_client.client_info.client_id == NULL || *wss_url == NULL) {
        ESP_LOGE(TAG, "Join response missing client_id/wss_url; treating as failed join");
        if (*wss_url) {
            free(*wss_url);
            *wss_url = NULL;
        }
    ...
    switch (event_id) {
        ...
        case WEBSOCKET_EVENT_DATA:
            if (!data || !data->data_ptr || data->data_len <= 0) {
                /* Zero-length data frames are normal keep-alive ping/pong
                 * traffic (ping_interval_sec=5) — not an error. */
                ESP_LOGD(TAG, "Ignoring empty WebSocket data event (keep-alive)");
                return;
            }
...
esp_err_t apprtc_signaling_disconnect(void)
{
    ...
    /* Tear down any active peer sessions first so their slots are freed. The
     * WS teardown below only drops signaling; the app_webrtc streaming sessions
     * outlive it unless we close them — otherwise each connect/disconnect leaks
     * a slot and the next offer fails with "Max streaming sessions reached".
     * Called outside the client mutex (it briefly waits on the cleanup loop). */
    app_webrtc_close_all_sessions();
