...
STATUS describeMediaStorageConfEsp(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus) && responseData != NULL) {
        ESP_LOGD(TAG, "Describe media storage config response: %s", responseData);
        ATOMIC_STORE(&pSignalingClient->result, (SIZE_T) SERVICE_CALL_RESULT_OK);

        // Parse the response — mirrors upstream describeMediaStorageConfLws.
        // Tracks pSignalingClient->mediaStorageConfig.{storageStatus, storageStreamArn}
        // and — critically — invalidates getEndpointTime on a storage-status flip so
        // the FILE_EXCEPT_DESCRIBE_MEDIA caching policy re-fetches the WEBRTC
        ...
        if (tokenCount > 1 && tokens[0].type == JSMN_OBJECT) {
            for (i = 1; i < tokenCount; i++) {
                if (!jsonInMediaStorageConfig) {
                    ...
                    if (compareJsonString(responseData, &tokens[i], JSMN_STRING, (PCHAR) "Status")) {
                        strLen = (UINT32) (tokens[i + 1].end - tokens[i + 1].start);
                        if (strLen <= MAX_ARN_LEN) {
                            newStorageStatus = (STRNCMP("ENABLED", responseData + tokens[i + 1].start, strLen) == 0);
                        ...
                        i++;
                    } else if (compareJsonString(responseData, &tokens[i], JSMN_STRING, (PCHAR) "StreamARN")) {
                        // StreamARN may be null (JSMN_PRIMITIVE) when storage is disabled.
                        if (tokens[i + 1].type != JSMN_PRIMITIVE) {
                            strLen = (UINT32) (tokens[i + 1].end - tokens[i + 1].start);
                            if (strLen <= MAX_ARN_LEN) {
                                STRNCPY(pSignalingClient->mediaStorageConfig.storageStreamArn,
                                        responseData + tokens[i + 1].start, strLen);
