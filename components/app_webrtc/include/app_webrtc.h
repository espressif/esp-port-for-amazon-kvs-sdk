...
/**
 * @brief Close all active streaming sessions without terminating the app
 *
 * Marks every active peer session for teardown and waits (bounded) for the
 * cleanup loop to free them and reset the session count. Call this when the
 * signaling link drops so the next connect starts from a clean slate — the app
 * itself (tasks, peer-connection client) stays initialized. Without it, each
 * connect/disconnect cycle leaks a session slot and offers eventually fail with
