...
WEBRTC_STATUS app_webrtc_init(app_webrtc_config_t *config)
{
    ...
    /* Require the core signaling lifecycle methods (a missing one should fail here, not silently
     * no-op later). ICE-refresh methods stay optional — apprtc implements them only partially. */
    if (config->signaling_client_if->init == NULL || config->signaling_client_if->connect == NULL ||
        config->signaling_client_if->disconnect == NULL || config->signaling_client_if->send_message == NULL ||
        config->signaling_client_if->free == NULL) {
        DLOGE("signaling_client_if is missing a required method (init/connect/disconnect/send_message/free)");
        CHK(FALSE, STATUS_INVALID_OPERATION);
    }
    ...
    /* Require the peer-connection lifecycle methods. create_session is intentionally NOT required:
     * the split-mode bridge peer-connection omits it, and that absence selects signaling-only mode. */
    if (config->peer_connection_if->init == NULL || config->peer_connection_if->free == NULL) {
        DLOGE("peer_connection_if is missing a required method (init/free)");
        CHK(FALSE, STATUS_INVALID_OPERATION);
    }
 ...
 */
WEBRTC_STATUS app_webrtc_close_all_sessions(void)
{
    PSampleConfiguration pSampleConfiguration = gSampleConfiguration;
    if (pSampleConfiguration == NULL) {
        return WEBRTC_STATUS_SUCCESS; /* nothing running */
    }

    /* Mark every active session for teardown. The session-cleanup loop
     * (sessionCleanupWait) does the actual free + streamingSessionCount--
     * once it sees terminateFlag; the broadcast wakes it immediately. Without
     * this, a signaling disconnect leaves sessions allocated and the count
     * pinned, so the next offer fails with "Max streaming sessions reached". */
    UINT32 marked = 0;
    MUTEX_LOCK(pSampleConfiguration->streamingSessionListReadLock);
    for (UINT32 i = 0; i < pSampleConfiguration->streamingSessionCount; ++i) {
        PAppWebRTCSession s = pSampleConfiguration->webrtcSessionList[i];
        if (s != NULL && !ATOMIC_LOAD_BOOL(&s->terminateFlag)) {
            ATOMIC_STORE_BOOL(&s->terminateFlag, TRUE);
            marked++;
        }
    }
    ...
    CVAR_BROADCAST(pSampleConfiguration->cvar);
    ESP_LOGI(TAG, "Marked %" PRIu32 " active session(s) for cleanup on disconnect", marked);

    /* Give the cleanup loop a bounded window to drain so an immediate rejoin
     * finds free slots. Cleanup is async; do not block forever. */
