...
{
    /* The PeerConnection ICE timer-queue executor runs the offer ->
     * setRemoteDescription -> gather -> candidate chain synchronously on this
     * thread on the libwebsockets (ws-off) signaling path. Its peak is ~16.9 KB
     * on the ICE connectivity + STUN + nomination + DTLS chain, so the 16 KB
     * DEFAULT_THREAD_SIZE overflows. Name it and give it 32 KB. */
    return timerQueueCreateEx(pHandle, (PCHAR) "kvsTimerQ", 32 * 1024);
}
